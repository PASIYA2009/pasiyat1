...
const BOT_NAME_FANCY = '𝐏𝐀𝐒𝐈𝐘𝐀 𝐌𝐃 𝐌𝐈𝐍𝐈';
...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://whatsapp.com/channel/0029VbC4A7X9Gv7ZiGIg6D1r',
  ...
  BOT_NAME: 'PASIYA MD MINI BOT',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'Pasidu ',
  IMAGE_PATH: 'https://files.catbox.moe/sup4af.jpeg',
  BOT_FOOTER: '𝐏𝐎𝐖𝐄𝐑𝐄𝐃 𝐁𝐘 𝐏𝐀𝐒𝐈𝐘𝐀 𝐌𝐃 𝐌𝐈𝐍𝐈 ',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/sup4af.jpeg' }
};

// ---------------- MONGO SETUP ----------------

const MONGO_URI = process.env.MONGO_URI || 'YOUR MONGO DB URL';
const MONGO_DB = process.env.MONGO_DB || 'ANY USER NAME'
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    let botName = cfg.botName || 'PASIYA MD MINI BOT';
...
case 'gpt': {
  try {
    ...
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'PASIYA MD MINI BOT AI';
...
ඔයාගේ නම ${botName} , ඔයාගෙන් කවුරුහරි "ඔයාව හැදුවේ කවුද?" කියලා ඇහුවොත් විතරක් ඔයා කියන්නේ "මාව හැදුවේ පසිඳු අයියා" කියලා. User Message: ${q}
    `;
...
case 'song': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'PASIYA MD MINI';
...
_© Powered by ${botName}_`;
 ...
 case 'video': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'PASIYA MD MINI';    // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '💖 PASIYA MD MINI 💖';
...
╭─❏ 🤖 BOT STATUS ❏
│ 👸 ${title}
│ 👑 Owner : ${config.OWNER_NAME || 'Pasidu Eshan'}
...
> © ${config.BOT_FOOTER || '💝 PASIYA MD MINI'}
`.trim();
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'PASIYA MD MINI BOT AI';
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'PASIYA MD MINI BOT AI'; // dynamic bot name
